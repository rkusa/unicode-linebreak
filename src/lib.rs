...
pub fn linebreaks_iter<'a>(
    iter: impl Iterator<Item = &'a str> + Clone + 'a,
    ...
    .chain(once((0, Right(once((0, eot as u8))))))
    .scan(0, |offset, (len, s)| {
        let o = *offset;
        *offset += len;
        Some(s.map(move |(i, bp)| (i + o, bp)))
    })
    .flatten()
    .scan((sot, false), |state, (i, cls)| {
        // ZWJ is handled outside the table to reduce its size
...
mod tests {
    ...
    fn linebreaks_iter_test() {
        ...
        assert_eq!(
            linebreaks_iter(vec!["foo ", "bar 🙄", " ✅"].into_iter()).collect::<Vec<_>>(),
            vec![
                (4, BreakOpportunity::Allowed),
                (8, BreakOpportunity::Allowed),
                (13, BreakOpportunity::Allowed),
